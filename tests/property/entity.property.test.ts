...
describe('Property 1: 엔티티 생성 및 조회', () => {
  it('모든 유효한 엔티티 입력에 대해, 엔티티를 생성한 후 이름으로 조회하면 동일한 정보가 반환되어야 함', () => {
    fc.assert(
      fc.property(entityInputArb, (input) => {
        const manager = new KnowledgeGraphManager();

        // 엔티티 생성
        const createResult = manager.createEntities([input]);
  ...
  it('여러 엔티티를 생성한 후 모두 조회할 수 있어야 함', () => {
    fc.assert(
      fc.property(
        ...
        (inputs) => {
          ...
          // 모든 엔티티 생성
          const createResult = manager.createEntities(inputs);
          expect(createResult.success).toBe(true);
